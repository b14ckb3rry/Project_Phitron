...
class Customer(User):
    ...
    def deposit(self, amount):
        if(amount > 0):
            ...
            print(f'Successfully Deposit and Updated Balance: {self.balance}.')
    ...
    def transfer(self, other, amount):
        if(amount > self.balance):
            ...
            print("Transfer Unsuccessful, Please, transfer positive amount!")
        else:
            self.balance -= amount
            if(other.bank != self.bank): self.bank -= amount
            other.recieve_transfer(amount)
            print(f'Transfer Successful and Updated Balance: {self.balance}.')
            self.transaction_history.append(f'TRANSFERED BAL: {amount} | CURRENT BAL: {self.balance}')
    
    
    def recieve_transfer(self, amount):
        self.balance += amount
        self.bank.balance += amount
        self.transaction_history.append(f'RECIEVED BAL: {amount} | CURRENT BAL: {self.balance}')
        return f'Transfer Recieved Successfully and Updated Balance: {self.balance}.'
...
class Admin(User):
    ...
    def view_total_balance(self):
        print("Total balance of bank:", self.bank.balance)
    
    def view_total_loan(self):
        print("Total Loan Amount of bank:", self.bank.loan_amount)
